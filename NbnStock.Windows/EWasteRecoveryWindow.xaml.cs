    ...
    public partial class EWasteRecoveryWindow : Window
    {
        ...
        private void InputScanner_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Return || e.Key == Key.Enter)
            {
                ...
                {
                    MessageBox.Show("This serial is already in your current recovery batch.", "Duplicate Scan", MessageBoxButton.OK, MessageBoxImage.Information);
                    InputScanner.Clear();
                    return;
                }
                ...
                if (existingUnit != null)
                {
                    ...
                    {
                        MessageBox.Show($"This unit is already logged in the E-Waste system. Current Status: {existingUnit.Status}", "Already Logged", MessageBoxButton.OK, MessageBoxImage.Warning);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        MessageBox.Show("This serial was not found in your database. Please select what type of hardware it is from the dropdown below before scanning.", "Legacy Unit Detected", MessageBoxButton.OK, MessageBoxImage.Warning);
                        return;
                    }
                ...
                if (RecoveryBatch.Count > 0)
                {
                    BatchDataGrid.ScrollIntoView(RecoveryBatch.Last());
                }
            }
        }
        ...
        private void BtnCommit_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                MessageBox.Show($"Successfully queued {RecoveryBatch.Count} units into the E-Waste pipeline.", "Recovery Complete", MessageBoxButton.OK, MessageBoxImage.Information);
                this.DialogResult = true;
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error committing to E-Waste: {ex.Message}", "Database Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
    }
}