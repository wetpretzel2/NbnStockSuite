    ...
    public partial class BulkConsumeWindow : Window
    {
        ...
        private void InputScanner_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Return || e.Key == Key.Enter)
            {
                ...
                if (selectedItem != null && !string.IsNullOrEmpty(serial))
                {
                    ...
                    {
                        MessageBox.Show("This serial is already queued for consumption!", "Duplicate Scan", MessageBoxButton.OK, MessageBoxImage.Warning);
                        InputScanner.Clear();
                        return;
                    }
                    ...
                    {
                        MessageBox.Show($"This serial belongs to a different item type. You selected: {selectedItem.Name}.", "Item Mismatch", MessageBoxButton.OK, MessageBoxImage.Warning);
                    }
        ...
        private void BtnAddConsumable_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (selectedItem != null && int.TryParse(InputQuantity.Text, out int qty) && qty > 0)
            {
                ...
                {
                    MessageBox.Show($"You do not have enough stock. Available: {selectedItem.Quantity}. Already queued: {currentlyQueued}.", "Insufficient Stock", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
        ...
        private void UpdateTotalCount()
        {
            ...
            {
                BatchDataGrid.ScrollIntoView(ConsumeBatch.Last());
            }
        }
        ...
        private void BtnCommit_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                MessageBox.Show($"Successfully recorded consumption for {ConsumeBatch.Count} entries.", "Consumption Complete", MessageBoxButton.OK, MessageBoxImage.Information);
                this.DialogResult = true;
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Database Error during commit: {ex.Message}", "Commit Failed", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
    }
}