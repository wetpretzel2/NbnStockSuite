...
public class StockRepository
{
    ...
    public void AdjustStockQuantity(int id, int changeAmount)
    {
        ...
        if (item == null) throw new Exception($"Stock item with Id {id} was not found.");

        var newQuantity = item.Quantity + changeAmount;

        if (newQuantity < 0) throw new Exception("Stock quantity cannot go below zero.");
    ...
    {
        if (quantityReceived <= 0) throw new Exception("Quantity received must be greater than zero.");
    ...
    {
        if (quantityUsed <= 0) throw new Exception("Quantity used must be greater than zero.");
